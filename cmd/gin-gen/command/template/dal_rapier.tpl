...
func (b {{$stName}}) DeleteByFilter(ctx context.Context, q *{{$queryPrefix}}Delete{{$stName}}ByFilter) (int64, error) {
    ref := {{$repoPrefix}}Ref_{{$stName}}()
    return ref.New_Executor(b.db).Model().
            Scopes(b.deleteInnerByFilter(ref, q)).
            Delete()
}
...
func (b {{$stName}}) GetByFilter(ctx context.Context, q *{{$queryPrefix}}Get{{$stName}}ByFilter, funcs ...DalCondition) (*{{$mdName}}, error) {
    ...
    return ref.New_Executor(b.db).Model().
            SelectExpr(ref.Select_Expr()...).
            Scopes(funcs...).
            Scopes(b.getInnerByFilter(ref, q)).
            TakeOne()
}
...
func (b {{$stName}}) ExistByFilter(ctx context.Context, q *{{$queryPrefix}}Exist{{$stName}}ByFilter, funcs ...DalCondition) (existed bool, err error) {
    ...
    return ref.New_Executor(b.db).Model().
            Scopes(funcs...).
            Scopes(b.existInnerByFilter(ref, q)).
            Exist()
}
...
func (b {{$stName}}) Count(ctx context.Context, q *{{$queryPrefix}}List{{$stName}}ByFilter) (total int64, err error) {
    ref := {{$repoPrefix}}Ref_{{$stName}}()
    return ref.New_Executor(b.db).Model().
            Scopes(b.listInnerByFilter(ref, q)).
            Count()
}
...
func (b {{$stName}}) List(ctx context.Context, q *{{$queryPrefix}}List{{$stName}}ByFilter) ([]*{{$mdName}}, error) {
    ...
    return ref.New_Executor(b.db).Model().
            SelectExpr(ref.Select_Expr()...).
            Scopes(b.listInnerByFilter(ref, q), Limit(q.Page, q.PerPage)).
            FindAll()
}
...
func (b {{$stName}}) PluckIdByFilter(ctx context.Context, q *{{$queryPrefix}}Pluck{{$stName}}ByFilter) ([]int64, error) {
    ref := {{$repoPrefix}}Ref_{{$stName}}()
    return ref.New_Executor(b.db).Model().
            Scopes(b.pluckInnerByFilter(ref, q)).
            PluckExprInt64(ref.Id)
}


func (b {{$stName}}) deleteInnerByFilter(ref *{{$repoPrefix}}{{$stName}}_Native, q *{{$queryPrefix}}Delete{{$stName}}ByFilter) func(db *gorm.DB) *gorm.DB {
...
func (b {{$stName}}) getInnerByFilter(ref *{{$repoPrefix}}{{$stName}}_Native, q *{{$queryPrefix}}Get{{$stName}}ByFilter) func(db *gorm.DB) *gorm.DB {
    return func(db *gorm.DB) *gorm.DB {
        {{- range $f := $e.Fields}}
            {{- if and (ne $f.GoName "CreatedAt") (ne $f.GoName "UpdatedAt") (ne $f.GoName "DeletedAt")}}
            {{- if eq $f.Type.Type 15 }}
...
func (b {{$stName}}) existInnerByFilter(ref *{{$repoPrefix}}{{$stName}}_Native, q *{{$queryPrefix}}Exist{{$stName}}ByFilter) func(db *gorm.DB) *gorm.DB {
    return func(db *gorm.DB) *gorm.DB {
{{- range $f := $e.Fields}}
    {{- if and (ne $f.GoName "CreatedAt") (ne $f.GoName "UpdatedAt") (ne $f.GoName "DeletedAt")}}
    {{- if eq $f.Type.Type 15 }}
...
func (b {{$stName}}) listInnerByFilter(ref *{{$repoPrefix}}{{$stName}}_Native, q *{{$queryPrefix}}List{{$stName}}ByFilter) func(db *gorm.DB) *gorm.DB {
    return func(db *gorm.DB) *gorm.DB {
{{- range $f := $e.Fields}}
    {{- if and (ne $f.GoName "CreatedAt") (ne $f.GoName "UpdatedAt") (ne $f.GoName "DeletedAt")}}
    {{- if eq $f.Type.Type 15 }}
...
func (b {{$stName}}) pluckInnerByFilter(ref *{{$repoPrefix}}{{$stName}}_Native, q *{{$queryPrefix}}Pluck{{$stName}}ByFilter) func(db *gorm.DB) *gorm.DB {
    return func(db *gorm.DB) *gorm.DB {
{{- range $f := $e.Fields}}
    {{- if and (ne $f.GoName "CreatedAt") (ne $f.GoName "UpdatedAt") (ne $f.GoName "DeletedAt")}}
    {{- if eq $f.Type.Type 15 }}
